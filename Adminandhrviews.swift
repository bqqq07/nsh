                            ...
                            VStack(spacing: 14) {
                                ...
                                HStack(spacing: 12) {
                                    KPICard(title: "معدل الإجازات",
                                            ...
                                            subtitle: "\(kpi.leave_count) يوم إجازة",
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                HStack(spacing: 12) {
                    Button { Task { await decide("rejected") } } label: {
                        Text("رفض").frame(maxWidth: .infinity).padding()
                            .background(Color(hex: "#e74c3c")).foregroundColor(.white).cornerRadius(14)
                    }
                    Button { Task { await decide("approved") } } label: {
                        Text("موافقة").frame(maxWidth: .infinity).padding()
...
struct HRInboxView: View {
    ...
    var filtered: [HRTask] {
        tab == "pending" ? tasks.filter { $0.status == "pending" } : tasks
    }
