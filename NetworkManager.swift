...
extension NetworkManager {
    ...
    func adminTraineeReport(traineeIds: [Int] = [], weekStart: String? = nil) async throws -> Data {
        var req = makeRequest("/api/hse/trainee-report/generate-auto", method: "POST")
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        var body: [String: Any] = ["trainee_ids": traineeIds]
        if let ws = weekStart { body["week_start"] = ws }
        req.httpBody = try JSONSerialization.data(withJSONObject: body)
        let (data, resp) = try await session.data(for: req)
    ...
    func ptwTrainingStatus() async throws -> PtwStatusResponse {
        let (data, _) = try await session.data(for: makeRequest("/api/ptw-training/status"))
        return try JSONDecoder().decode(PtwStatusResponse.self, from: data)
    }

    func ptwModuleDoors(modSeq: Int) async throws -> PtwModuleDoorsResponse {
        let (data, _) = try await session.data(for: makeRequest("/api/ptw-training/module/\(modSeq)/doors"))
        return try JSONDecoder().decode(PtwModuleDoorsResponse.self, from: data)
    }

    func ptwDoorDetail(modSeq: Int, doorSeq: Int) async throws -> PtwDoorDetail {
        let (data, _) = try await session.data(for: makeRequest("/api/ptw-training/module/\(modSeq)/door/\(doorSeq)"))
        return try JSONDecoder().decode(PtwDoorDetail.self, from: data)
    }

    func ptwSubmitDoor(modSeq: Int, doorSeq: Int, answers: [String], photoData: Data?, photoExt: String) async throws -> [String: String] {
        var req = makeRequest("/api/ptw-training/module/\(modSeq)/door/\(doorSeq)/submit", method: "POST")
        let boundary = UUID().uuidString
        req.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        var body = Data()
        let answersJSON = (try? JSONSerialization.data(withJSONObject: answers)) ?? Data("[]".utf8)
        body.append("--\(boundary)\r\nContent-Disposition: form-data; name=\"answers\"\r\n\r\n".data(using: .utf8)!)
        body.append(answersJSON)
        body.append("\r\n".data(using: .utf8)!)
        if let pd = photoData {
            let mime = photoExt == "png" ? "image/png" : "image/jpeg"
            body.append("--\(boundary)\r\nContent-Disposition: form-data; name=\"photo\"; filename=\"photo.\(photoExt)\"\r\nContent-Type: \(mime)\r\n\r\n".data(using: .utf8)!)
            body.append(pd)
            body.append("\r\n".data(using: .utf8)!)
        }
        body.append("--\(boundary)--\r\n".data(using: .utf8)!)
    ...
    func ptwSupervisorPending() async throws -> [PtwPendingItem] {
        let (data, _) = try await session.data(for: makeRequest("/api/ptw-training/supervisor/pending"))
        return try JSONDecoder().decode([PtwPendingItem].self, from: data)
    }

    func ptwSupervisorReview(subId: Int, action: String, note: String) async throws -> [String: String] {
        var req = makeRequest("/api/ptw-training/supervisor/review/\(subId)", method: "POST")
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        req.httpBody = try? JSONSerialization.data(withJSONObject: ["action": action, "note": note])
        let (data, _) = try await session.data(for: req)
        return (try? JSONDecoder().decode([String: String].self, from: data)) ?? [:]
    }

    // ── Welfare / Environment API ────────────────────────────────────

    func welfareObservations(page: Int = 1) async throws -> WelfareObsPage {
        let (data, _) = try await session.data(for: makeRequest("/api/welfare/observations?page=\(page)"))
        return try JSONDecoder().decode(WelfareObsPage.self, from: data)
    }

    func welfareCreateObservation(params: [String: String]) async throws -> [String: Int] {
        var req = makeRequest("/api/welfare/observation", method: "POST")
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
    ...
    func welfareUploadObsPhoto(obsId: Int, imageData: Data, ext: String) async throws {
        var req = makeRequest("/api/welfare/observation/\(obsId)/photo", method: "POST")
        let boundary = UUID().uuidString
        req.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        var body = Data()
        let mime = ext == "png" ? "image/png" : "image/jpeg"
        body.append("--\(boundary)\r\nContent-Disposition: form-data; name=\"photo\"; filename=\"obs.\(ext)\"\r\nContent-Type: \(mime)\r\n\r\n".data(using: .utf8)!)
        body.append(imageData)
        body.append("\r\n--\(boundary)--\r\n".data(using: .utf8)!)
        req.httpBody = body
        _ = try await session.data(for: req)
    ...
    func welfareReports() async throws -> [WelfareReportItem] {
        let (data, _) = try await session.data(for: makeRequest("/api/welfare/reports"))
        return try JSONDecoder().decode([WelfareReportItem].self, from: data)
    }

    func welfareUploadReport(pdfData: Data, filename: String, reportType: String, reportDate: String, notes: String) async throws -> [String: Int] {
        var req = makeRequest("/api/welfare/report/upload", method: "POST")
        let boundary = UUID().uuidString
        req.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        var body = Data()
        ...
        addField("report_date", reportDate)
    ...
    func welfareViewReport(rid: Int) -> URL {
        let token = SessionManager.shared.token ?? ""
        return URL(string: "\(BASE_URL)/api/welfare/report/\(rid)?token=\(token)")!
    }

    func welfareDeleteReport(rid: Int) async throws {
        let req = makeRequest("/api/welfare/report/\(rid)", method: "DELETE")
        _ = try await session.data(for: req)
    }

    func welfareSupervisorAll(role: String = "", from: String = "", to: String = "") async throws -> WelfareSupervisorData {
        var path = "/api/welfare/supervisor/all?"
        if !role.isEmpty { path += "role=\(role)&" }
        if !from.isEmpty { path += "from=\(from)&" }
        if !to.isEmpty   { path += "to=\(to)&" }
        let (data, _) = try await session.data(for: makeRequest(path))
        return try JSONDecoder().decode(WelfareSupervisorData.self, from: data)
    }
...
struct PtwStatusResponse: Codable {
    let active: Bool
    let modules: [PtwModuleSummary]
}

struct PtwModuleSummary: Codable, Identifiable {
    var id: Int { seq }
    let seq: Int
    let title: String
    let title_ar: String
...
struct PtwModuleDoorsResponse: Codable {
    let module_seq: Int
    let title: String
    let doors: [PtwDoorDetail]
}

struct PtwDoorDetail: Codable, Identifiable {
    var id: Int { seq }
    let seq: Int
    let title: String
    let ref: String
...
struct PtwPendingItem: Codable, Identifiable {
    let id: Int
    let officer_id: Int
    let officer_name: String
    let module_seq: Int
    ...
    let door_seq: Int
    let door_title: String
    let answers: [String]
    let photo_path: String
    let submitted_at: String?
}

// ── Welfare / Environment Models ─────────────────────────────────────

struct WelfareObsItem: Codable, Identifiable {
    let id: Int
    let date: String
    let location: String
    let obs_type: String
    ...
    let closed_at: String?
    let closure_action: String
    let photos: [WelfarePhoto]
}

struct WelfarePhoto: Codable {
    let path: String
    let photo_type: String
}

struct WelfareObsPage: Codable {
    let items: [WelfareObsItem]
    let page: Int
    let pages: Int
    let total: Int
}

struct WelfareReportItem: Codable, Identifiable {
    let id: Int
...
struct WelfareSupervisorData: Codable {
    let observations: [WelfareObsItem]
    let reports: [WelfareSupervisorReport]
}

struct WelfareSupervisorReport: Codable, Identifiable {
    let id: Int
    let report_type: String
    let report_date: String
    let role_type: String
